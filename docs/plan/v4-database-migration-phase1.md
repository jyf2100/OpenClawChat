...
- [x] 设置页已暴露消息导入状态与手动校验入口
- [x] 已具备房间级全量校验命令
- [x] 已完成“最近一页消息灰度切读”的评估结论，并暂缓启用
- [x] `npm run build` 通过
...
- 已新增房间级全量校验命令：
  ...
  - 对比房间全部消息的数量、缺失、额外和内容不一致 ID
- 对“最近一页消息灰度切读”的评估结论：
  - 当前暂不启用
  - 原因：导入和校验能力刚建立，仍需先观察稳定性和错误率
- 仍未切任何消息读路径，启动链路不受影响。
...
- 抽样校验与全量校验都已具备，但仍缺少长期统计沉淀与告警阈值。
- 最近一页消息灰度切读尚未真正实现，只完成了风险评估和暂缓决定。

### 下一个最小任务

- 为消息导入增加长期统计沉淀（成功率/失败率/不一致率）。
- 设计“最近一页消息灰度切读”的开关与回滚机制，再决定是否启用。
